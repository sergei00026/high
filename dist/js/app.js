(() => {
    ...
    function checkScreenWidth() {
        if (window.innerWidth <= 991) initSliders();
    }
    checkScreenWidth();
    window.addEventListener("resize", checkScreenWidth);
    const script_anchors = document.querySelectorAll(".link");
    for (let anchor of script_anchors) anchor.addEventListener("click", (function(e) {
        e.preventDefault();
        const blockID = anchor.getAttribute("href").substr(1);
        if (anchor.closest(".menu-open")) anchor.closest(".menu-open").classList.remove("menu-open", "lock");
